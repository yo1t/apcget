...
- 両 opener に `Connection: close` ヘッダーを付与し、リクエストごとに接続を切断することで PowerChute 側の Tomcat のコネクション蓄積を防止する
...
- `alreadyLoggedOn` HTML 要素 ID の存在のみで判定
- 言語非依存：表示テキストに依存しない
...
- リダイレクト先（`/status`）の HTML を返す（新しいロケールでのステータスページ）

#### `restore_locale(opener, base_url, original_locale)`

- ステータス取得中に変更したロケールを元に戻す
...
- 現在の表示言語を検出し、英語以外であれば `_set_locale` で英語に切り替え、その戻り値の HTML を直接利用する（余分なリクエストを省略）
- `original_locale`（切替前のロケールコード、または既に英語の場合は `None`）を返し、後で復元に使用する
...
- IP アドレス/ホスト名（`ip`、`--zabbix-send`）: `_validate_host()` で `^[\w.\-\[\]:]+$` パターンを検証（URL インジェクション / SSRF 防止）
- Zabbix ホスト名: `_sanitize_zabbix_host()` で `^[\w.\-]+$` パターンを追加検証（コマンドインジェクション防止）
- Zabbix 送信値: 改行文字を除去。スペースを含む値（例: `"On Line"`）はダブルクォートで囲む
...
- 環境変数による指定もサポート
- `mosquitto_pub` 使用時に `--mqtt-password` のパスワードがプロセス一覧（`ps aux`）に露出する。`paho-mqtt` はこの問題がない
...
| 既存セッション検出 | `alreadyLoggedOn` の存在（要素 ID のみ） | HTML 要素 ID は言語不変。表示テキストによる判定は不要 |
